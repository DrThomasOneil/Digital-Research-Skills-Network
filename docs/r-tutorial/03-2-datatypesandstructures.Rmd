...
Some functions or operations require data to the correct data type. For example, math can't be performed on a character
...
# numeric
num = "1"
typeof(num) # 1 is in quotes - so classified as a char
...
To demonstrate a logical, a good example of their use is to check a condition and run a script. I'll use `if` here, which we cover in the next chapter.
...
Most commonly, you'll use `data <- read.csv()` or `data <- readRDS()`. 
